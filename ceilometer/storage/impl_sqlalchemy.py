...
def make_query_from_filter(query, event_filter, require_meter=True):
    ...
    # sqlite doesn't compare time strings well, convert to datetime
    ...
    if event_filter.start:
        ts_start = sanitize_timestamp(event_filter.start)
        query = query.filter(Meter.timestamp >= ts_start)
    if event_filter.end:
        ts_end = sanitize_timestamp(event_filter.end)
...
class Connection(base.Connection):
    ...
    def make_volume_query(self, event_filter, counter_volume_func):
    ...
    def get_volume_sum(self, event_filter):
        ...
        query = self.make_volume_query(event_filter, counter_volume_func)
        results = query.all()
